...
interface RehireScreenProps {
  onNext: () => void;
  onPrev: () => void;
}

const RehireScreen: React.FC<RehireScreenProps> = ({ onNext, onPrev }) => {
  const { t } = useLanguage();
  const { npsData, setRehireScore } = useNPS();

  const handleSelectScore = (score: number) => {
    setRehireScore(score);
    onNext();
  };
        ...
        {Array.from({ length: 10 }, (_, i) => i + 1).map((score) => (
          <motion.button
            ...
            className={`number-button ${
              npsData.rehireScore === score ? "selected" : ""
            }`}
          >
            {score}
          </motion.button>
